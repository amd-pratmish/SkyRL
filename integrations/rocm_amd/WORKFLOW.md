...
Typical smoke (Qwen2.5-0.5B, GSM8K subset, 2 GPUs): generate with sync `vLLM.LLM`, sync weights with `weight_sync_backend=nccl`, train with `trainer.strategy=megatron`.
...
Inside `main_base`: Ray init with explicit `num_gpus` → vLLM Ray actors (`vllm.LLM`, one per GPU) → Megatron policy + ref on the same placement group → GRPO loop (generate → reward → forward → advantage → weight sync → train step).
...
| Area | Change |
|------|--------|
| `vllm_import_compat.py` | Import shims for moved OpenAI entrypoints |
| `vllm_engine.py` | Keep HIP/ROCR/CUDA-compatible device masks aligned |
| `utils.py` | HIP-gated `VLLM_USE_V1=0` and `VLLM_TARGET_DEVICE=rocm` |
| GRPO config | `async_engine=false`, `enable_sleep_mode=false` |
...
The GSM8K recipe uses `_SKYRL_USE_NEW_INFERENCE=0`:
...
4. `skyrl_start_weight_update` → `load_weights` per chunk → `skyrl_finish_weight_update`
